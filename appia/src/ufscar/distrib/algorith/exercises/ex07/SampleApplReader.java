  ...
  public void run() {
    while (true) {
      try {
         ...
         * channel.
         */
        P2PSendableEvent asyn = new P2PSendableEvent();
        Message message = asyn.getMessage();
        try{
        	//Recebe entrada do id do processo de destino e adiciona como recipient
        	asyn.setRecipient(Integer.parseInt(st.nextToken())); //<nr_proceso>        	
        }catch (Exception e) {
			// TODO: handle exceptionparentSession
        	e.printStackTrace();
		}
        String msg = "";
        ...
        asyn.asyncGo(parentSession.getChannel(), Direction.DOWN);
